...
- **Images.** Every `<img>` carries a non-empty `alt`.
...
- **Palette restraint.** Keep the `<style>` block to at most six distinct non-grayscale colors across `color`, `background-color`, `border-color`, and `--` custom properties.
